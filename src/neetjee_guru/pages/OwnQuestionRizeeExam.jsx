...
import OwnQuestionRizeeExamSection from '../components/questioners/create_question_paper/own_question_paper/own_question_types/OwnQuestionRizeeExamSection';
...
class OwnQuestionRizeeExam extends Component {
    constructor(props) {
        super(props)
        this.state = {
            currentStep: 1,
    ...
    handleInputChange = (e) => {
        ...
        if (e.target.name == "counttype") {
            ...
            if (e.target.value != "ocount") {
                // sData = this.state.questionsubjects.map((Data) => {
    ...
    subjectFunction = (subjectid, type) => {
        let subjectarray = [];
        if (type == "subject") {
            console.log("subjectFunction", subjectid);
            subjectarray = this.state.subjects.map((submapData) => {
            ...
            console.log("subjectFunction", subjectid);
            subjectarray = this.state.questionsubjects.map((submapData) => {
                if (submapData.id == subjectid) {
                    return { ...submapData, subjectactive: "d-flex justify-content-between align-items-center active" }
                }
                return { ...submapData, subjectactive: "d-flex justify-content-between align-items-center" }
            })
            this.setState({ questionsubjects: subjectarray });
        }


    }
    squestioncount = (e, subid, type) => {
        let arr = [];
        if (type == "qcount") {
            arr = this.state.questionsubjects.map(item => {
            ...
            arr = this.state.questionsubjects.map(item => {
                ...
                if (item.id == subid) {
                    const re = /^[0-9,]+$/;
                    if (re.test(e.target.value)) {
                        return { ...item, questions: e.target.value, qtotError: "" };
                    } else {
                        return { ...item, questions: "", qtotError: "Invalid Input" }
                    }
                }
                return item;
            }
        ...
        this.setState({ questionsubjects: arr });
    }
    ...
    render() {
        console.log("pcurrentstate", this.state, Cookies.get("institutionid"));
