...
class PreviousPaperDataTable extends Component {

    constructor(props){
        console.log("questionTypeComplexity", props.questionTypeComplexity);
        super(props);
      this.state={
           renderArray: []
    ...
    imageFunction(item){
        ...
        <span className="ml-2 title">{item}</span>
        </div>);
    ...
    componentDidMount=()=>{
        let renderArray = [];
        if (this.props.questionTypeComplexity != undefined) {
           
            if (this.props.questionTypeComplexity[0] != undefined) {
                renderArray = [
                    {
                        row: '0',
                        rowSpan: '2',
                ...
                this.props.questionTypeComplexity[0].subject.map((item, index) => {
                    let col=item.year.length+1;
                            const newObj1 = {
                                row: '0',
                                rowSpan: '',
                                colSpan: col,
                                ...
                                dataSort: false,
                                img:this.imageFunction(item.subject)
                            }
                            ...
                            renderArray.push(newObjt2);
                            
                            console.log("item.year",item.year);
                            item.year.map((ymap, index1)=>{
                                let first_word = ymap.year.split("-")[0];
                                
                            const newObj2 = {
                                    row: '1',
                                    rowSpan: '',
                                    colSpan: '',
                                    dataField: item.subject+ymap.year + index1,
                                    dataSort: true,
                                    isKey: false,
                                    width: "25%",
                                    headerAlign: '',
    ...
    products(data){
        ...
        if (data != undefined) {
            data.map((item) => {
                 ...
                 newObj = {
                    ...
                    tottotal:item.qsCount

                }
                item.subject.map((smap)=>{
                    if(smap!=undefined){
                        const field1 = smap.subject+"total";
                        //console.log("parseInt(smap.qsCount)",parseInt(smap.qsCount));
                        let newObj2 = {
                            ...newObj,
                            [`${field1}`]: parseInt(smap.qsCount),
                           
                           
                        }
                        newObj = newObj2;

                        smap.year.map((ymap, index1)=>{
                             //for total percentage
                            let mtotarry=[];
                            data.map((dmap)=>{
                                dmap.subject.map((ssmap)=>{
                                    if(ssmap.subject==smap.subject){
                                         let totarry=[];
                                         ssmap.year.map((a)=>{
                                            if(ymap.year==a.year){
                            ...
                            //end total percentage
                                 ...
                                 const field2 = smap.subject+ymap.year + index1;
                                const field2p = (parseInt(ymap.count) / parseInt(sum) * 100);
                                let val = "";
                                if (isNaN(field2p)) {
                                    val = 0;
                                }
                                ...
                                let newObj1 = {
                                        ...newObj,
                                        [`${field2}`]: parseInt(ymap.count)+" " + val + "%",
        ...
        console.log("nproductsewArray", newArray);
        return newArray;
    ...
    render() {
        ...
        const options = {
            ...
            sizePerPageList: [{
                ...
                text: 'All', value: this.props.questionTypeComplexity.length
        ...
        if(this.state.renderArray.length==0){
            return(null);
        }
        return (
            <BootstrapTable data={this.products(this.props.questionTypeComplexity)} pagination={true} options={options}>
                    ...
                    if(item.colSpan!=""){
                        ...
                        return( <TableHeaderColumn 
                            ...
                            isKey={item.isKey}
                             width={item.width}
                             headerAlign={item.headerAlign}
                        >{item.img}{item.subject}</TableHeaderColumn>);
                    }
...
export default PreviousPaperDataTable
