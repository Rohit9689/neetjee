...
import { Container, Row, Col, Nav, Navbar, NavDropdown, Badge, Accordion, Card, Image, Button } from 'react-bootstrap'
...
const GET_PRACTISE_TYPES = gql`
  query(
          $mobile: String) {
            getPracticeTypes(mobile: $mobile){
                practice_name
                total_questions
                duration
                short_description
...
class NewPracticeInstructions extends Component {
    constructor(props) {
        super(props)
    ...
    render() {
        ...
        const getPracticeTypes = this.props.getPracticeTypes;
        const loading1 = getPracticeTypes.loading;
        const error1 = getPracticeTypes.error;
        if (loading1) {
            return <PreloaderTwo />
        }
        if (error1 !== undefined) {
        ...
        let singlePracticeType = "";
        if (getPracticeTypes.getPracticeTypes != undefined) {
            singlePracticeType = getPracticeTypes.getPracticeTypes.find((a) => a.practice_name == "Rapid Practice");
        }

        console.log("singlePracticeType", singlePracticeType);

        const instructionsCard = [
            {
                id: 1,
                img: 'https://images.unsplash.com/photo-1585399000684-d2f72660f092?ixid=MXwxMjA3fDF8MHxlZGl0b3JpYWwtZmVlZHw2fHx8ZW58MHx8fA%3D%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60',
                title: 'Rapid Practice',
                description: 'Ut enim ad minim veniam, quis nostrud exercitation ullamco aliquip commodo consequat.',
                duration: '20 min',
                questions: '40',
                marks: '100'
            },
        ...
        return (
            <div className="PracticeInstructions">
                <Navbar className="header-top navbar navbar-expand-md navbar-white bg-white shadow-sm">
                    <Container>
                        ...
                        <Nav className="ml-auto">
                            <NavDropdown id="basic-nav-dropdown1" className="student-notification no-arrow" align="end" title={
                                ...
                                <ul>
                                    ...
                                    <li>
                                        <Link className="pt-2 nav-link text-center text-dark" to="/notifications"> All
                                            notifications</Link>
                                    </li>
                                </ul>
                            </NavDropdown>
                            <div className="profile-img d-flex align-items-center">
                                ...
                                <NavDropdown id="basic-nav-dropdown2" className="no-arrow" align="end" title={
                                    <i className="fal fa-ellipsis-v fa-2x text-dark pt-1 mx-2" />
                                }>
                                    ...
                                    <NavDropdown.Divider />
                                    <NavDropdown.Item as={Link} to="/student/login"><i className="far fa-sign-out mr-2" /> Logout</NavDropdown.Item>
                                </NavDropdown>
                            </div>
                        </Nav>
                    </Container>
                ...
                <div className="content-box pt-4" style={{ paddingBottom: '4rem' }}>
                    <Container>
                        <Row>
                            <Col xl={12} lg={12} md={12} sm={12} xs={12}>
                                <Accordion className="practice-instructions-card" defaultActiveKey={this.state.currentActiveKey}>
                                    <Accordion.Item eventKey={"1"} className="border-0">
                                        <Card.Header className="bg-white p-2">
                                            <Row className="align-items-center">
                                                <Col xl={3} lg={3} md={6} sm={12}>
                                                    <Image src="https://images.unsplash.com/photo-1585399000684-d2f72660f092?ixid=MXwxMjA3fDF8MHxlZGl0b3JpYWwtZmVlZHw2fHx8ZW58MHx8fA%3D%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60" rounded="true" fluid="true" />
                                                </Col>
                                                <Col xl={9} lg={9} md={6} sm={12}>
                                                    <Row>
                                                        <Col xl={7} lg={6} md={12} sm={12}>
                                                            <div className="content pb-3">
                                                                <div className="h4">{singlePracticeType.practice_name}</div>
                                                                <p className="d-none d-lg-block light-color">{singlePracticeType.short_description}</p>
                                                            </div>
                                                        </Col>
                                                        <Col xl={5} lg={6} md={12} sm={12}>
                                                            <Row className="pb-3">
                                                                <Col xs={4}>
                                                                    <div className="h6 light-color font-weight-normal">Duration</div>
                                                                    <div className="h4 font-weight-normal">{singlePracticeType.duration}</div>
                                                                </Col>
                                                                <Col xs={4}>
                                                                    <div className="h6 light-color font-weight-normal">Questions</div>
                                                                    <div className="h4 font-weight-normal">{singlePracticeType.total_questions}</div>
                                                                </Col>
                                                                <Col xs={4}>
                                                                    <div className="h6 light-color font-weight-normal">Marks</div>
                                                                    <div className="h4 font-weight-normal">{singlePracticeType.marks}</div>
                                                                </Col>
                                                            </Row>
                                                        </Col>
                                                        <Col xl={{ span: 7, order: 3 }} lg={{ span: 6, order: 3 }} md={{ span: 12, order: 4 }} sm={{ span: 12, order: 4 }} xs={{ span: 12, order: 4 }} >
                                                            <Accordion.Header as={Button} variant="link text-dark font-weight-bold text-decoration-none d-flex align-items-center px-0"
                                                                eventKey={"1"}
                                                                onClick={() => { this.setState({ currentActiveKey: "1" }) }}
                                                            >
                                                                <span className="border-bottom">Please read the instructions carefully</span>
                                                                <i className={
                                                                    this.state.currentActiveKey === "1" ?
                                                                        'far fa-chevron-down ml-2' : 'far fa-chevron-right ml-2'
                                                                } />
                                                            </Accordion.Header>
                                                        </Col>
                                                        <Col xl={{ span: 5, order: 4 }} lg={{ span: 6, order: 4 }} md={{ span: 12, order: 3 }} sm={{ span: 12, order: 3 }} xs={{ span: 12, order: 3 }} className="text-lg-right text-center">
                                                            <Button
                                                                variant="primary"
                                                                onClick={() => {
                                                                    localStorage.setItem("subjectid", localData.subjectid);
                                                                    localStorage.setItem("type", localData.type);
                                                                    localStorage.setItem("ocid", localData.ocid);
                                                                    localStorage.setItem("otid", localData.otid);
...
export default
    graphql(GET_PRACTISE_TYPES,
        {
            options: props => ({
                variables: {
                    mobile: Cookies.get("mobile"),
                },
            }), name: "getPracticeTypes"
        })(NewPracticeInstructions);