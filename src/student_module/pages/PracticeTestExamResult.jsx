...
import PracticeTestExamResultDetails from "../components/exam_result/PracticeTestExamResultDetails";
...
import MockTests from '../components/exam_result/MockTests'
...
`;
class PracticeTestExamResult extends Component {
    constructor(props) {
        super(props)

        this.state = {
            modalShow: false,
            depethreportShow: true
        }
    }
    componentDidMount = () => {
        console.log("this.props.history.location.state.examtype", this.props.history.location.state.examtype);
        if (this.props.history.location.state.examtype == "series_test") {
            const title = GoogleAnalyticsArray[0].MockTest_Results;
            ReactGA.pageview('/student/subject/exam-result', ["ELAPP"], title);
        }
        if (this.props.history.location.state.examtype == "previous_exam") {
            const title = GoogleAnalyticsArray[0].Previous_Exam_Results;
            ReactGA.pageview('/student/subject/exam-result', ["ELAPP"], title);
        }
        if (this.props.history.location.state.examtype == "schedule_exam") {
            const title = GoogleAnalyticsArray[0].Schedule_Exam_Reports;
            ReactGA.pageview('/student/subject/exam-result', ["ELAPP"], title);
        }
        if (this.props.history.location.state.examtype == "custom") {
            const title = GoogleAnalyticsArray[0].Custom_Exam_Reports;
            ReactGA.pageview('/student/subject/exam-result', ["ELAPP"], title);
        }
        if (this.props.history.location.state.examtype == "error_exam") {
            const title = GoogleAnalyticsArray[0].Error_Exam_Reports;
            ReactGA.pageview('/student/subject/exam-result', ["ELAPP"], title);
        }
        if (this.props.history.location.state.examtype == "jeemainsprevious_exam") {
            const title = GoogleAnalyticsArray[0].Jee_Mains_2021_Results;
            ReactGA.pageview('/student/subject/exam-result', ["ELAPP"], title);
        }

        const title = GoogleAnalyticsArray[0].Exam_Result;
        ReactGA.pageview('/student/subject/exam-result', ["ELAPP"], title);


    }
    complexityColorThemeFun(ctype) {
        if (ctype == "Easy") {
            return "text-success";

        }
        else if (ctype == "Moderate") {
            return "text-warning";

        }
        else if (ctype == "Difficult") {
            return "text-danger";

        } else if (ctype == "Highly Difficult") {
            return "theme-red";

        }

    }
    complexityIconFun(ctype) {
        if (ctype == "Easy") {
    ...
    totalmarksFun(object) {

        if (object != undefined) {
            //let tot_marks = (parseInt(object.correct) + parseInt(object.wrong) + parseInt(object.not_answered)) * parseInt(object.pMarks);
            let tot_marks = (parseInt(object.correct) + parseInt(object.wrong) + parseInt(object.not_answered)) * parseInt(object.pMarks);
            if (!isNaN(tot_marks)) {
                return tot_marks
            }
    ...
    correctmarksFun(object) {
        let data = "";
        if (object != undefined) {
            let correct_marks = parseInt(object.correct) * parseInt(object.pMarks);
            let negative_marks = (parseInt(object.wrong) - parseInt(object.noNegativeCount)) * parseInt(object.nMarks);
            data = parseInt(correct_marks) - parseInt(negative_marks);
        }
        if (!isNaN(data)) {
            return data
        }
    ...
    examNameFun() {
        console.log("this.props.history.location.state.examtype", this.props.history.location.state.examtype);
        if (this.props.history.location.state.examtype == "schedule_exam" || this.props.history.location.state.examtype == "schedule" || this.props.history.location.state.examtype == "schedulehome") {
    ...
    render() {
                        ...
                        <Row className="align-items-center my-3">
                            <Col className="d-flex justify-content-between align-center">
                                <div className="title-block">
                                    <h5 className="title mb-0">Your Exam Result</h5>
                                ...
                                <div className="right-content d-flex align-items-center">
                                    {this.props.history.location.state.type == "live" ?
                                        (<Link className="d-flex align-items-center rounded-pill btn px-3 py-2 ml-2" style={{ background: '#212b64', color: '#fff', border: '1px solid #212b64' }} onClick={() => this.windowClose()}> Back</Link>)
                                        : (
                                            <React.Fragment>
                                                {this.props.history.location.state.examtype == "series_test" ? (
                                                    <Link className="d-flex align-items-center rounded-pill btn px-3 py-2 ml-2" style={{ background: '#212b64', color: '#fff', border: '1px solid #212b64' }} to={{
                                                        pathname: "/student/exams/test-series"
                                                    }}> Back</Link>
                                                ) :
                                                    this.props.history.location.state.examtype == "jeemainsprevious_exam" ? (
                                                        <Link className="d-flex align-items-center rounded-pill btn px-3 py-2 ml-2" style={{ background: '#212b64', color: '#fff', border: '1px solid #212b64' }} to={{
                                                            pathname: "/student/createpreviouspaperexam"
                                                        }}> Back</Link>
                                                    )
                                                        : (
                                                            <Link className="d-flex align-items-center rounded-pill btn px-3 py-2 ml-2" style={{ background: '#212b64', color: '#fff', border: '1px solid #212b64' }} to={{
                                                                pathname: "/student/subject/exam-history", state: {
                                                                    examtype: this.props.history.location.state.examtype
                                                                }
                                                            }}> Back</Link>
                                                        )}
                                            </React.Fragment>
                                        )}
                                    {/* {this.props.history.location.state.mocktype == "history" ? (
                                    ...
                                    {this.props.history.location.state.examtype == "series_test" ? (
                                        <React.Fragment>
                                            {/* {localStorage.getItem("mocklivestatus") == "live" ? (
                                            <React.Fragment> */}
                                            {condition == true || this.props.history.location.state.mocktype != "history" ? (
                                                <React.Fragment>
                                                    <Link
                                                        to={{
                                                            pathname: "/student/view-question-answer",
        ...
        oneday = 3600 + moment.unix(getStudentExamSessions.getStudentExamSessions[0].timestamp).endOf('day').unix();
        condition = (moment().unix() <= oneday) ? true : false;
        ...
        let stotq = 0;
        getStudentExamSessions.getStudentExamSessions[0].skipped_vs_complexity.map((cmap) => {
                ...
                <Container>
                    <Row className="align-items-center my-3">
                        <Col className="d-flex justify-content-between align-center">
                            ...
                            <div className="right-content d-flex align-items-center">
                                {this.props.history.location.state.type == "live" ? (<Link className="d-flex align-items-center rounded-pill btn px-3 py-2 ml-2" style={{ background: '#212b64', color: '#fff', border: '1px solid #212b64' }} onClick={() => this.windowClose()}> Back</Link>)
                                    : (<React.Fragment>
                                        {this.props.history.location.state.examtype == "series_test" ? (
                                            <Link className="d-flex align-items-center rounded-pill btn px-3 py-2 ml-2" style={{ background: '#212b64', color: '#fff', border: '1px solid #212b64' }} to={{
                                                pathname: "/student/exams/test-series"
                    ...
                    <Row className="mt-4">
                        <Col xl={2} lg={6} md={6} sm={6} xs={6} className="my-3">
                            <Card as={Card.Body} className="score-card bg-darkblue border-0 pt-2 text-center text-white shadow">
                                <ul className="list-inline dot-circle mb-3">
                                ...
                                <h6 className="card-subtitle">Score</h6>
                                <h1 className="card-title mb-0">{this.graphValue()} <small>/ {this.props.getStudentExamSessions.getStudentExamSessions[0].total_marks}</small></h1>
                            </Card>
                        </Col>
                        <Col xl={2} lg={6} md={6} sm={6} xs={6} className="my-3">
                            <Card as={Card.Body} className="avgtime-card bg-warning border-0 pt-2 text-center shadow">
                                <ul className="list-inline dot-circle mb-3">
                                    <li className="list-inline-item"><i className="fas fa-circle" /><i className="fas fa-circle" /><i className="fas fa-circle" /><i className="fas fa-circle" /></li>
                                    <li className="list-inline-item"><i className="fas fa-circle" /><i className="fas fa-circle" /><i className="fas fa-circle" /><i className="fas fa-circle" /></li>
                                    <li className="list-inline-item"><i className="fas fa-circle" /><i className="fas fa-circle" /><i className="fas fa-circle" /><i className="fas fa-circle" /></li>
                                </ul>
                                <h6 className="card-subtitle">AVG Time / Q</h6>
                                <h1 className="card-title mb-0">{getStudentExamSessions.getStudentExamSessions[0].speed} <small> Sec</small></h1>
                            </Card>
                        </Col>
                                ...
                                <Row className="g-0">
                                    {getStudentExamSessions.getStudentExamSessions[0].subject_report.map((subjectmap) => {
                                        return (
                                            <Col>
                                                <Card
                                                    className={this.subjectClassfun(subjectmap.id)}>
                                                    ...
                                                    <Card.Body className="rounded p-3">
                                                        <h6 className="card-subtitle pt-2">{subjectmap.subject}</h6>
                                                        <h1 className="card-title mb-0">{this.correctmarksFun(subjectmap)} <small>/ {this.totalmarksFun(subjectmap)}</small></h1>
                                                        {/* <h1 className="card-title mb-0">{subjectmap.correct} <small>/ {parseInt(subjectmap.correct) + parseInt(subjectmap.wrong)}</small></h1> */}
                                                    </Card.Body>
                                                </Card>
                                            </Col>
                                        );
                        ...
                        <Col xl={12} lg={12} md={12}>
                            <Card className="maintable mt-3 shadow">
                                <Table className="mb-0 borderless" responsive>
                                    <thead>
                                        <tr>
                                            <th></th>
                                            <th className="theme-darkblue">
                                                <div className="d-flex align-items-center">
                                                    <div className="icon">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="14.015" height="24.162" viewBox="0 0 14.015 24.162"><defs></defs><g transform="translate(-497.192 -406.784)"><path fill="#212b64" d="M322.353,43.249l-2.956.456a6.37,6.37,0,0,1,2.4-7.674,7.123,7.123,0,0,1,8.56.041,6.574,6.574,0,0,1,2.113,7.907c-.478,1.277-1.275,2.444-1.671,3.738a31.418,31.418,0,0,0-.693,4.2l-2.685.812c-.275-1.323-.874-2.569-.688-3.685.287-1.721,1.3-3.312,1.647-5.032a9.955,9.955,0,0,0-.022-4.654c-.186-.614-2.4-1.357-2.92-.979-1.195.876-1.931,2.389-2.813,3.672A4.226,4.226,0,0,0,322.353,43.249Z" transform="translate(178.219 372.161)" /><path fill="#212b64" d="M327.954,54.064l3.27-.988,1.218,4.034-3.214,1.171Z" transform="translate(178.465 372.665)" /></g></svg>

                                                    </div>
                                                    <div className="ml-3">Total<br /> Questions</div>
                                                </div>
                                            </th>
                                            <th className="text-dark">
                                                <div className="d-flex align-items-center">
                                                    ...
                                                    <div className="ml-3">AVG <br />Time/ Q</div>
                                                </div>
                                            </th>
                                            {getStudentExamSessions.getStudentExamSessions[0].correct_vs_complexity.map((complexitymap) => {
                                                return (<th className={this.complexityColorThemeFun(complexitymap.complexity)}>
                                                    <div className="d-flex align-items-center">
                                                        <div className="icon">
                                                            {this.complexityIconFun(complexitymap.complexity)}

                                                        </div>
                                                        <div className="ml-3">{complexitymap.complexity} <br /> Questions</div>
                                                    </div>
                                                </th>)
                                            })}
                                        </tr>
                    ...
                    {this.props.history.location.state.examtype == "series_test" ? (
                        <React.Fragment>
                            {localStorage.getItem("mocklivestatus") == "live" ? (<MockTests />) : ("")}
                        </React.Fragment>) : ("")}



                    <Row>
                        <Col xl={12} lg={12} md={12} className="my-4 text-center">
                            <Button className="btn btn-darkblue rounded-pill px-4 text-capitalize" onClick={() => { this.setState({ depethreportShow: !this.state.depethreportShow }) }}>
                                {
                                    this.state.depethreportShow ? 'Click Here to Get Detailed Analysis (In depth report)' : 'Hide Detailed Analysis (In depth report)'
                                }
                            </Button>
                        </Col>

                        {
                            this.state.depethreportShow ? null :
                                <PracticeTestExamResultDetails exam_session_id={this.props.history.location.state.sessionid} />
