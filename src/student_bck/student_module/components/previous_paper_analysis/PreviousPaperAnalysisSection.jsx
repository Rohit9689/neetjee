...
import QuestionTypeComplexity from './QuestionTypeComplexity';
    ...
    render() {
        ...
        if (loading1) {
            ...
            const getPreviousPaperAnalysisData1 = getPreviousPaperAnalysis.getPreviousPaperAnalysis.subjects_weightage.map((smap) => {
                if (smap != undefined) {
                    const singleSubject = this.props.globalsubjects.find((a) => a.id == smap.id);
                    const chaptersArr = smap.chapters.map((cmap) => {
                        if (cmap != undefined) {
                            const singleChapter = singleSubject.studentChapters.find((a) => a.id == cmap.id);
                            const topicArr = cmap.topic.map((tmap) => {
                                if (tmap != undefined) {
                            ...
                            if (singleChapter != undefined) {
                                return { ...cmap, chapter: singleChapter.chapter, topic: topicArr }
                            }
                    ...
                    if (singleSubject != undefined) {
                        return { ...smap, subject: singleSubject.subject, chapters: chaptersArr }
                    }
            ...
            if (getPreviousPaperAnalysisData.isYearEmpty == true) {
                return (
                    <div className="previous-paper-analysis">
                        <Container>
                            <Row className="my-4">
                                ...
                                <Col xl={12} lg={12} md={12} sm={12} xs={12}>
                                    ...
                                    <Accordion defaultActiveKey={this.state.topdefaultActiveKey}>
                                            ...
                                            <ChapterWeightage
                                                ...
                                                type="Grouping"
                                                getPreviousPaperAnalysis={getPreviousPaperAnalysisData} />
                                        ...
                                        {getPreviousPaperAnalysisData.questionTheoryWise.length > 0 ? (
                                            <Card className="mt-4 question-theory">
                                                <Accordion.Toggle as={Card.Header} eventKey="3" className="bg-white"
                                                    ...
                                                    <Card.Title className="h6 mb-0 px-2">
                                                        ...
                                                        {this.state.topdefaultActiveKey != "3" ? (
                                                            <i className="fa-fw far fa-plus-square mr-2" style={{ cursor: "pointer" }}></i>
                                                                ...
                                                                Question Theory (Concept / Application) Wise Weightage</Card.Title>
                                                </Accordion.Toggle>
                                                    ...
                                                    <Card.Body>
                                                        ...
                                                        <Row>
                                                            <Col xl={12} lg={12} md={12} sm={12} className="previous_complexity_wise">
                                                                {getPreviousPaperAnalysisData.questionTheoryWise.length > 0 ? (
                                                                    <CardLessDataTable
                                                                        articlecolumns={this.subjectQuestionTheoryColumns(getPreviousPaperAnalysisData.questionTheoryWise, "questionTheroy")}
                                                                        articledata={this.subjectQuestionTheoryData(getPreviousPaperAnalysisData.questionTheoryWise, "questionTheroy", "0")}
