...
import { subjectCardData } from '../StudentHomeData';
import SubjectCardsLoader from './SubjectCardsLoader';
...
class SubjectCards extends Component {
    ...
    subjectClass(data) {
        ...
        if (data.subject == "Botany") {
            classname = `single-card shadow-sm botany ${data.active}`
        }
        else if (data.subject == "Physics") {
            classname = `single-card shadow-sm physics ${data.active}`
        }
        else if (data.subject == "Chemistry") {
            classname = `single-card shadow-sm chemistry ${data.active}`
        }
        else if (data.subject == "Zoology") {
            classname = `single-card shadow-sm zoology ${data.active}`
        }
        else if (data.subject == "Mathematics") {
            classname = `single-card shadow-sm maths ${data.active}`
        }
        return classname;
    ...
    ProgressBar() {
        ...
        let count = Math.round(oldcount / this.props.getSubjects.length);
        console.log("count", count);
        return count;
    }
    render() {
        console.log("SubjectCards", this.props.getSubjects);
        return (
            <Row className="student-subject-card">
                {
                    this.props.getSubjects.map((item, index) => {
                        console.log("item.practice_percentage", item);
                        return (
                            // this.state.isLoading ?
                            //     <SubjectCardsLoader />
                            //     :

                            <Col xl={3} lg={3} md={6} sm={12} key={item.id}>
                                <Card
                                    //className={`single-card shadow-sm ${subjectName}`}
                                    onClick={() => this.props.handleSelect(index)}
                                    className={this.subjectClass(item)}
                                >
                                    <Card.Body className="p-2">
                                        <div className="d-flex align-items-center mt-2 mb-3">
                                            <Card.Subtitle>
                                                {this.iconFun(item.subject)}
                                            </Card.Subtitle>
                                            <Card.Title className="ml-3 text-white">{item.subject}</Card.Title>
                                        </div>
                                        <div className="text-center">
                                        Practise Questions Completed - {item.practice_percentage} %
                                        </div>
                                        <ProgressBar now={item.practice_percentage} variant="white"
                                            // label={`${item.practice_percentage}%`}
                                            min={0} max={100} />

                                        <div className="d-flex justify-content-between align-items-center">
                                            <p className="mb-0 text-white">0%</p>
                                            <p className="mb-0 text-white">100%</p>
                                        </div>
                                    </Card.Body>
                                </Card>
                            </Col>
