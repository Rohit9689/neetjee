...
const FETCH_PRACTICE_GRAPH = gql`
  query($mobile: String!) {
    getPracticeGraph(mobile: $mobile) {
      id
      subject
      viewseries{
        type
...
`;
class LearnPracticeSection extends Component {
  constructor(props) {
    super(props);
  ...
  topclassName = (data) => {
    ...
    } else if (data == "Revision Materials") {
      ...
      return (
        ...
        >
          ...
          <path
            ...
            transform="translate(-6.843 -72.001)"
          ></path>
          <path
            fill="#fff"
            d="M65.614,81.77c.808,2.823.021,4.756-2.121,5.387a3.578,3.578,0,0,1-4.53-2.539c-.6-2.125.73-3.857,3.63-4.671-.057-1.413.436-2.7,1.959-2.8a3.136,3.136,0,0,1,2.408,1.247C67.874,79.879,66.9,81.009,65.614,81.77Z"
            transform="translate(-37.99 -49.818)"
          ></path>
          <path
            fill="#fff"
            d="M69.837,46.722c-.691.457-1.456,1.349-2.058,1.254a2.436,2.436,0,0,1-1.64-1.742c0-.591,1.038-1.573,1.7-1.65.609-.071,1.335.836,2.008,1.314C69.848,46.173,69.841,46.449,69.837,46.722Z"
            transform="translate(-42.716 -28.788)"
          ></path>
          <path
            fill="#fff"
            d="M68.659,27.36c.623.776,1.332,1.307,1.25,1.64-.127.51-.8.885-1.243,1.317-.393-.414-1.052-.8-1.1-1.25C67.515,28.649,68.114,28.157,68.659,27.36Z"
            transform="translate(-43.634 -17.667)"
          ></path>
        </svg>
      );
    } else if (data == "Physics") {
  ...
  ProgressBar(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      let idata = data[i];
      const sdata = idata.practice_percentage;
      total.push(parseInt(sdata));
    }
    for (let num of total) {
      oldcount = oldcount + num;
    }
    let count = Math.round(oldcount / data.length);
    return count;
  }
  render() {
    if (this.props.getPracticeGraph.loading) return null;
    console.log("this.props.getPracticeGraphsub", this.props.getPracticeGraph.getPracticeGraph);
    ...
    if (isuserValid.learn_tab == true) {
      return (
        <section className="learn_practice">
          <Row>
      ...
      return (
        <section className="learn_practice">
          <div className="title-block mb-4">
            <h5 className="title mb-0">Learn &amp; Practice -   <span className="small text-gray4">
              Select Subjects to start learning &amp; Practice
          </span></h5>

          </div>
          <Row>
            <Col xl={12} lg={12} md={12} sm={12} xs={12}>
              <Card className="learn-card-status border-0">
                <Card.Header className="bg-white py-3">
                  <h6>Overall Practice Completion</h6>
                  <ProgressBar
                    variant="success"
                    now={this.ProgressBar(this.props.getSubjects)}
                    label={`${this.ProgressBar(this.props.getSubjects)}%`}
                  />
                ...
                <Card.Body className="p-2">
                  <div className="bg-white border-0 d-flex justify-content-between align-items-center p-2">
                    <p className="mb-0">Practice Report</p>
                    <div className="weekselection">
                      <SelectDropDown
                        options={weekdays}
                        placeholderName={"weekdays"}
                        handleChange={this.handleSelectInputChange}
                        dropdownIndicator={{ DropdownIndicator }}
                      />
                    </div>
                  </div>
                  <MainLineChart timeseries={this.state.timeseries} titleHeading="Views Graph" />
                </Card.Body>
              </Card>
            </Col>
          </Row>
          <Row className="mt-4">
            {this.props.getSubjects.map((getSubjectsdata, index) => {
              let chaptername = getSubjectsdata.studentChapters.find(
                (a) => a.id == getSubjectsdata.last_attempted_chapter
              );
              let cname = "";
              if (chaptername != undefined) {
                cname = chaptername.chapter;
              }
              const views = this.props.getPracticeGraph.getPracticeGraph.find((a) => a.id == getSubjectsdata.id);
              return (
                <Col xl={4} lg={6} md={6} sm={6} xs={12}>
                  {/* <Link
                    to={{
                      ...
                      state: {
                        //chapters1: getSubjectsdata.studentChapters,
                        //subject: getSubjectsdata.subject,
                        subjectid: getSubjectsdata.id,
                        //getSubjects: this.props.getSubjects,
                        //subjectindex: index,
                        //practice_percentage: getSubjectsdata.practice_percentage,
                        //htitle: "Chapters",
                        //htitlecount: getSubjectsdata.studentChapters.length,
                        totquestions: this.getTotalQuestions(
                          getSubjectsdata.studentChapters
                        )
                      }
                    }}
                  > */}
                  <Card
                    className="singlecard border-0 shadow-sm"
                  //className={this.topclassName(getSubjectsdata.subject)}
                  >
                    <div
                      className={this.className(getSubjectsdata.subject)}
                    ></div>
                    <Card.Header className="p-0">

                      <div className="content d-flex align-items-center">
                        <div className="icon">
                          {this.classNameIcon(getSubjectsdata.subject)}
                        </div>
                        <div className="content-block2 pl-4">
                          <h5>{getSubjectsdata.subject}</h5>
                          <h6 className="text-white">
                            {getSubjectsdata.practice_percentage}%{" "}
                    ...
                    <Card.Body className="py-3">
                      {views.viewseries.map((view) => {
                        return (
                          <div className="snv"><i className={this.topclassName(view.type)} /> {view.type} Views : <span className="font-weight-bold">{view.value} / {view.total}</span></div>
                        )
